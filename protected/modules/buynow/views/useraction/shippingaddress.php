				 ...
				 <?php $this->renderPartial('application.modules.useractivity.views.useraction.profilemenu',array('user'=>$user,
					...
					<?php if(Yii::app()->controller->action->id == 'shippingaddress') { ?>
							...
							<div class="profile-tab-content-heading col-xs-12 col-sm-12 col-md-12 col-lg-12 no-hor-padding">
								...
								<a data-target="#address-modal" data-toggle="modal" href="#" class="profile-tab-heading-btn pull-right">
									<div class="btn-follow btn-chat-with-seller col-xs-12 col-sm-12 col-md-3 col-lg-2 no-hor-padding">
										<div class="chat-with-seller-text col-xs-12 col-sm-12 col-md-12 col-lg-12 no-hor-padding" onclick="clear_add_address();">+<?php echo Yii::t('app','Add address'); ?></div>
									</div>
								</a>
							</div>
						...
						<?php if(empty($address)) {
												...
												<span class="payment-red">'.Yii::t('app','Sorry...').'</span> '.Yii::t('app','You haven’t added any shipping address yet.').'</div>';
						} else { ?>

							<?php foreach($address as $add){ ?>

									<div class="joysale-acc-addr-cont col-xs-12 col-sm-6 col-md-6 col-lg-4" style="float:left;">
										...
										?>
											...
											<?php $userDefaultAddress = Myclass::getDefaultShippingAddress($add->userId);
											if($add->shippingaddressId == $userDefaultAddress) {
													echo '<a href="" class="joysale-acc-continue-btn joysale-acc-btn joysale-acc-select">'.Yii::t('app','Default').'</a>';
												}
												else
												{
													$defaulturl = Yii::app()->createAbsoluteUrl('buynow/useraction/default',array('id'=>$add->shippingaddressId,'userid' => $add->userId));
													echo '<a href="'.$defaulturl.'" class="joysale-acc-continue-btn joysale-acc-btn joysale-acc-select setdefault_btn">'.Yii::t('app','Set as default').'</a>';
...
<div class="paypal-form-container"></div>
    ...
    'action'=>Yii::app()->createUrl('/addshipping'),
