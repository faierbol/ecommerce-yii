 ...
 *
 * The followings are the available columns in table 'hts_photos':
...
class Photos extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('productId, name, createdDate', 'required'),
			array('productId, createdDate', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>60),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('photoId, productId, name, createdDate', 'safe', 'on'=>'search'),
		);
	}
